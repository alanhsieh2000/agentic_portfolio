...
- Candidates listed on non-U.S. exchanges can be added using Yahoo Finance's suffix notation, for example `7203.T` for Tokyo or `BARC.L` for London. Each portfolio must use a single currency: the first ticker added establishes it, and a ticker priced in a different currency is refused by name rather than silently mixed in, because prices in two different units cannot be allocated against one budget. So a yen portfolio and a dollar portfolio are built as two separate runs, with `--value` interpreted in the portfolio's own currency (shown as the `Portfolio currency` line in the output). Exchanges that quote in a minor unit are normalized to the major one, so London's pence become pounds. All of these pools are kept together in memory/candidates.json, one per currency, and when more than one is saved the system lists them and asks which to resume, or lets you start a new one in another currency. Converting between currencies is not implemented; that would be required only for a single portfolio holding several currencies at once.
