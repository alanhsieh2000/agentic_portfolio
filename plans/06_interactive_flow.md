...
- [ ] Give `run_pipeline` and `run_full_backtest` a `selection: str = "llm_s_only"` parameter (literal values `"llm_s_only"`, `"llm_f_only"`, `"llm_s_and_f"`) per README's Backtest Mode Stage 1 sentence "LLM-F only selection and LLM-S + LLM-F selection are all expensive evaluation. LLM-S only selection is the default selection." In `"llm_s_only"` mode, skip the LLM-F call (`screen_month`) entirely — not just ignore its result — and call `scan_with_detail(llm_s_signals, None)`; in `"llm_f_only"` mode, skip LLM-S's `generate_rule`/`screen` and call `scan_with_detail(None, llm_f_signals)`; in `"llm_s_and_f"` mode, call both as originally specified below. `src/scanner/candidate_scanner.py`'s `scan_with_detail` already has the three matching branches (`llm_s_only`/`llm_f_only`/`intersection`/`union`, confirmed correct in `plans/08_consistency_review.md` finding 12) — only `run_pipeline`'s call-site logic needs to conditionally build or omit each agent's call and signal argument. See `plans/08_consistency_review.md` finding 9 for the full rationale.
