...
- [x] (2026-08-14) Measured whether `yfinance`'s `Ticker.info` actually exposes usable AUM/total-net-assets and price-to-book fields for ETFs live, against 9 real ETFs spanning distinct categories (`SPY`, `QQQ`, `VOO`, `VTI` — large-blend/growth index funds; `PFFA` — preferred stock, this plan's own worked example; `AGG` — bond; `GLD` — commodity; `XLF` — sector-equity; `JEPI` — derivative income; `ARKK` — actively-managed growth), using the repository's pinned `yfinance==1.5.2`. Result recorded in Surprises & Discoveries below: `totalAssets`/`netAssets` (AUM) are present and usable for all 9/9 ETFs tested; `priceToBook` is present for 6/9 but genuinely absent (not zero — the key itself is missing from `info`) for `PFFA`, `JEPI`, `ARKK`. This plan's acceptance did not depend on this measurement (`compute_raw_factors_for_etf` already works correctly with manually-supplied numbers, unchanged by this finding); the recommendation this measurement produces is recorded in the Decision Log and Outcomes below rather than an auto-fetch implementation, which remains explicitly out of scope for this plan.
- [x] (2026-08-14) Researched and implemented free alternatives for the `priceToBook`/AUM gap the previous measurement found, then wired both into `compute_raw_factors_for_etf` as auto-fetch defaults (superseding the same day's earlier "leave it fully manual" decision — see Decision Log). Added `aggregate_price_to_book` (pure, unit-tested weighted-harmonic-mean aggregator), `estimate_price_to_book_from_holdings` (fetches an ETF's top ~10 disclosed holdings via `yf.Ticker(...).get_funds_data().top_holdings` and aggregates each usable holding's own `priceToBook`), and `_auto_fetch_etf_aum`/`_auto_fetch_etf_price_to_book` (the `Ticker.info`-first, holdings-fallback-second auto-fetch helpers) to `src/agents/external_screen.py`. `compute_raw_factors_for_etf`'s `aum`/`price_to_book` parameters are now optional — auto-fetched when omitted, used as-is (unchanged behavior) when the caller supplies them explicitly. 4 new hand-fixture unit tests added to `tests/test_external_screen.py` (11 total, all passing, no network calls); full repo suite (`uv run pytest tests/`, 118 tests) still passes. Verified end-to-end with real network calls, `aum`/`price_to_book` both omitted: `PFFA` -> `{'mve': 21.62, 'bm': None, 'mom12m': 0.042}` (AUM auto-fetched; `bm` correctly stays null — see Surprises & Discoveries for why no proxy exists for this fund); `JEPI` -> `{'mve': 24.55, 'bm': 0.120, 'mom12m': 0.018}` (both auto-fetched, `bm` implying an estimated `price_to_book` of 8.30 from 17.2% holdings coverage); `ARKK` -> `{'mve': 22.44, 'bm': 0.131, 'mom12m': -0.061}` (`price_to_book` estimate 7.62 from 47.7% coverage).
...
- Measurement (2026-08-14): live `yfinance.Ticker(ticker).info` (yfinance 1.5.2, this repository's pinned version per `pyproject.toml`) for 9 real ETFs across distinct fund categories. `totalAssets` and `netAssets` (two independent AUM fields, always numerically consistent with each other in every case checked) are present for all 9/9: `SPY` ($795.3B), `QQQ` ($452.8B), `VOO` ($1.687T), `VTI` ($2.290T), `AGG` ($137.1B), `GLD` ($130.3B), `XLF` ($57.9B), `JEPI` ($45.8B), `ARKK` ($5.56B), and — notably — `PFFA` ($2.4545B), within 1% of the $2.48B figure this plan's own Concrete Steps Step 2 example had to look up manually from Morningstar, confirming `totalAssets` is both present and accurate enough to replace that manual step. `priceToBook` is present for 6/9 (`SPY`, `QQQ`, `VOO`, `VTI`, `AGG`, `GLD`, `XLF`) but the key is genuinely absent from `info` (not zero, not null — missing entirely) for the other 3: `PFFA` (preferred stock), `JEPI` (derivative income / options-overlay), and `ARKK` (actively-managed growth). All three gaps share a plausible common cause: price-to-book is a well-defined aggregate only for a fund whose holdings are mostly common equity with a conventional book value — preferred-stock, options-overlay/derivative-income, and non-benchmark-tracking active-growth funds are exactly the categories where that assumption breaks down, so the gap looks like a real category-driven limitation in the underlying Yahoo Finance data feed itself, not a `yfinance`-specific parsing bug (the same asymmetry — AUM universally available, price-to-book only for conventional equity-holding funds — would likely reproduce against Yahoo Finance's own website for the same tickers).


- Research (2026-08-14): investigated three candidate alternatives for the `priceToBook` gap found in the previous measurement, all via live `yfinance==1.5.2` calls.
  1. `yf.Ticker(t).get_funds_data().equity_holdings["Price/Book"]` exists for every ETF tested, including all 3 gap tickers — but is **not the same metric** as `Ticker.info["priceToBook"]` and must not be used as a substitute. Cross-checked against 4 ETFs where both fields exist: `SPY` (1.81 vs 0.19, ratio 9.4x), `QQQ` (2.05 vs 0.12, ratio 17.1x), `VOO` (1.83 vs 0.19, ratio 9.9x), `XLF` (1.71 vs 0.42, ratio 4.1x) — four different, inconsistent ratios rule out a simple unit/scaling bug; this is a differently-defined statistic (plausibly a mislabeled Morningstar style-box widget value in `yfinance`'s HTML-scraping-based fund overview scraper, a different code path from the JSON `quoteSummary` API `.info` uses).
  2. A fund's own price-vs-NAV ratio (`Ticker.info["navPrice"]` vs `regularMarketPrice`) is not a useful substitute either, for a conceptual reason rather than a data-quality one: ETF arbitrage keeps a fund's market price pinned close to its NAV regardless of what the fund holds, so this ratio stays near 1.0 and carries no information about whether the fund's *underlying portfolio* is cheap or expensive — the actual quantity `bm`'s proxy is meant to capture per this plan's existing Decision Log.
  3. `yf.Ticker(t).get_funds_data().top_holdings` (the fund's top ~10 disclosed positions with weights) plus each holding's own `Ticker.info["priceToBook"]`, combined via a weighted-harmonic-mean aggregate (`sum(weight) / sum(weight / pb)` — the same "aggregate price / aggregate book" method index providers use, not a plain average of ratios) IS viable, but only for funds whose top holdings are ordinary common stocks. Verified live for `JEPI` and `ARKK`: all 10 top holdings each resolved as `quoteType == "EQUITY"` with a usable `priceToBook` (`AMZN`, `MSFT`, `TSLA`, etc., one holding — `ABBV` — with a real, well-documented negative `priceToBook` of roughly -75 from its own negative book equity, correctly down-weighted rather than distorting the result by the harmonic-mean method). This recovered `bm` for both: `JEPI`'s auto-fetched `price_to_book` estimate is 8.30 (17.2% of AUM covered by the top-10, since JEPI is a broadly-diversified large-cap fund), `ARKK`'s is 7.62 (47.7% covered, since ARKK is much more concentrated). Verified NOT viable for `PFFA`: its top holdings are individual preferred-share issues (`STRC`, `FLGPA`, `KKRPD`, `TDSPV`, `BANCPF`, `RLJPA`, `GNLPD`, `ORCLPD`, `TWOPC`, `ETPI`) — 8 of the 10 sampled don't even resolve as tickers via `yfinance` (HTTP 404, "Quote not found"), confirmed reproducible; this is not merely a coverage gap to work around, since preferred shares trade relative to a fixed par/liquidation value rather than a residual claim on book equity the way common stock does, so "price-to-book" has no standard meaning for a preferred-stock fund's holdings in the first place. `estimate_price_to_book_from_holdings` encodes this as a documented heuristic (return `None` unless at least half of the fund's disclosed top holdings resolve as ordinary equities with a usable ratio) rather than trying to force a number out of 1-2 resolvable holdings, which would be lone-holding-driven and unrepresentative, not a real estimate.
  Net conclusion: alternative (3), the top-holdings weighted-harmonic-mean estimate, is the one implemented (see Progress and the superseding Decision Log entry below); alternatives (1) and (2) are documented here specifically so a future contributor doesn't re-discover and re-reject them from scratch.
...
- Decision: leave `compute_raw_factors_for_etf` requiring caller-supplied `aum`/`price_to_book` unchanged — do not build an auto-fetch path from `yfinance.Ticker.info` — despite the 2026-08-14 measurement confirming `totalAssets` is reliably available. **Superseded later the same day — see the next Decision Log entry.**
  Rationale: the measurement's own result is the reason not to build a single auto-fetch path rather than the reason to build one — `totalAssets` alone would only ever supply half of `compute_raw_factors_for_etf`'s two required inputs, since `priceToBook` is missing for exactly the fund categories (preferred-stock, derivative-income, actively-managed) most likely to be screened as an off-index candidate in practice, not a random 1/3 of ETFs. An auto-fetch path would need to silently fall back to manual entry for `bm` alone whenever `priceToBook` is absent while auto-fetching `mve` alone — a half-automated, half-manual function is more surprising to a caller than the current fully-manual one, for a benefit (skipping one Morningstar lookup) this plan's Outcomes already judged small. If a future plan wants partial automation, this measurement's per-field breakdown (not a blanket "yfinance doesn't have it") is the correct starting point.
  Date/Author: 2026-08-14, plan author, decided while executing this plan's last pending Progress item.
- Decision: supersede the above — auto-fetch both `aum` and `price_to_book` in `compute_raw_factors_for_etf` by default (via new `_auto_fetch_etf_aum`/`_auto_fetch_etf_price_to_book` helpers), with explicit caller-supplied values still taking full precedence when given.
  Rationale: the prior decision's specific objection — that automating `aum` alone while leaving `price_to_book` structurally un-automatable would be a confusing half-measure — no longer holds once `price_to_book` has partial automatic coverage too. The user explicitly asked to find alternatives for the `priceToBook` gap; the resulting research (see Surprises & Discoveries) found a real, if partial, one: a weighted-harmonic-mean estimate from an ETF's top disclosed holdings recovers `price_to_book` for equity-holding funds (`JEPI`, `ARKK`) while correctly declining to guess for funds where the concept doesn't apply (`PFFA`, preferred-stock). Both fields auto-fetch with graceful degradation to `None` on any failure, and a caller who has better information than any automatic source (e.g. this plan's original manually-entered PFFA figures) can still override either or both — this is a strict superset of the old fully-manual interface, not a replacement for it, so no existing caller behavior changes unless it was already passing `None`.
  Date/Author: 2026-08-14, plan author, decided in response to the user's explicit request to find alternatives, implemented the same day.
...
As of 2026-08-14, that measurement is done: `yfinance.Ticker.info`'s AUM fields (`totalAssets`/`netAssets`) are reliably present and accurate (confirmed against this plan's own PFFA example, within 1% of the Morningstar figure originally entered by hand); its `priceToBook` field is not — present for conventional equity-index/sector funds but absent for preferred-stock, derivative-income, and actively-managed growth funds, which are disproportionately the kind of niche, off-benchmark fund this plan's `screen_external_candidate` exists to evaluate in the first place.

Later the same day, asked to find alternatives for that `priceToBook` gap specifically, a further research pass found and implemented a real, if partial, fix: a fund's top ~10 disclosed holdings, each holding's own individual `priceToBook`, combined via a weighted-harmonic-mean aggregate (the statistically correct way to combine a valuation ratio across holdings — a plain average would let one extreme ratio like AbbVie's real, negative `priceToBook` distort the result out of proportion to its actual weight). This recovers `price_to_book` automatically for equity-holding funds (`JEPI`, `ARKK` — both previously gaps, now auto-fetched with plausible values) while correctly declining to guess for funds where the concept has no standard meaning at all (`PFFA`, a preferred-stock fund — preferred shares trade relative to par value, not book equity, so there is no reliable free *or paid* source for this, not merely an unmeasured one). `compute_raw_factors_for_etf`'s `aum`/`price_to_book` parameters are now optional, auto-fetching via this tiered approach when omitted while remaining fully overridable when a caller supplies a trusted figure — a strict superset of the original manual-only interface, not a replacement for it. This plan's every Progress item is complete, with no remaining open questions; the auto-fetch gap is now closed as far as it structurally can be, given that "price-to-book" genuinely does not apply to every fund category.
...
Real output from `compute_raw_factors_for_etf(ticker, date(2023, 12, 1))` with `aum`/`price_to_book` both **omitted** (auto-fetch path), captured 2026-08-14, superseding the manual-only PFFA transcript above as the primary way to call this function (the manual path shown above still works unchanged, for a caller who wants to override with a trusted figure):

    PFFA -> {'mve': 21.62117945939082, 'bm': None, 'mom12m': 0.0419869067808194}
    JEPI -> {'mve': 24.54756319749816, 'bm': 0.12049826955490563, 'mom12m': 0.018205785338018865}
    ARKK -> {'mve': 22.439271252176212, 'bm': 0.13122538581699705, 'mom12m': -0.06056783230835827}

    (INFO log lines emitted during the run:)
    JEPI: Ticker.info lacks priceToBook; using holdings-based estimate 8.299 (coverage=17.2% of AUM)
    ARKK: Ticker.info lacks priceToBook; using holdings-based estimate 7.620 (coverage=47.7% of AUM)

`PFFA`'s auto-fetched `mve=21.62` corresponds to `aum≈$2.454B` — within 1% of the $2.48B figure manually entered in the transcript above, confirming the auto-fetch path reproduces the same real-world number without requiring a manual Morningstar lookup. `PFFA`'s `bm` correctly stays `None` (`estimate_price_to_book_from_holdings` found only 2 of its 10 disclosed top holdings resolvable as ordinary equities, below the half-usable threshold) rather than guessing — a caller wanting a `bm` for `PFFA` must still supply `price_to_book` manually, exactly as before. `JEPI`/`ARKK`'s `bm` values are new: both auto-fetched via the holdings-based estimate, implying `price_to_book` of 8.30 and 7.62 respectively — both plausible for a large-cap dividend/income fund and a growth/tech fund, and both notably lower than a naive (non-harmonic) average of the same 10 holdings' ratios would have given (~10.4 and ~11.7, computed and discarded during this research pass), illustrating why the harmonic-mean method matters in practice, not just in theory.
...
In `src/agents/external_screen.py`: `def compute_raw_factors_for_ticker(ticker: str, as_of_date) -> dict[str, float | None]`; `def compute_raw_factors_for_etf(ticker: str, as_of_date, aum: float | None = None, price_to_book: float | None = None) -> dict[str, float | None]` (both proxy inputs optional as of 2026-08-14 — auto-fetched via `_auto_fetch_etf_aum`/`_auto_fetch_etf_price_to_book` when omitted, used as-is when supplied); `def aggregate_price_to_book(weighted_pb: list[tuple[float, float]]) -> float | None` (pure weighted-harmonic-mean helper, unit-tested); `def estimate_price_to_book_from_holdings(ticker: str) -> tuple[float | None, float]` (the holdings-based fallback, network-calling, manually verified); `def standardize_raw_factors(raw: dict[str, float | None], stats: dict[str, tuple[float, float]]) -> dict[str, float]`; `def screen_external_candidate(rule: ScreeningRule, raw_factors: dict[str, float | None], rebalance_date, db_path: str = DEFAULT_DB_PATH) -> str`, returning one of `"buy"`, `"sell"`, `"hold"`, `"insufficient_data"`. Any future plan wanting to test whether an off-index candidate belongs in the portfolio's candidate set (extending `plans/04_candidate_scanner.md`'s scope, or a future interactive-flow feature in the spirit of `plans/06_interactive_flow.md`) can call `screen_external_candidate` directly, the same way `plans/04_candidate_scanner.md` calls `plans/02_llm_s_agent.md`'s in-universe `screen`.
