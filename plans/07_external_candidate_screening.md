...
- Decision: leave `compute_raw_factors_for_etf` requiring caller-supplied `aum`/`price_to_book` unchanged — do not build an auto-fetch path from `yfinance.Ticker.info` — despite the 2026-08-14 measurement confirming `totalAssets` is reliably available.
