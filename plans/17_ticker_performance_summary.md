...
- [x] (2026-09-22 08:24Z) Threaded the interactive window's effective minimum into ticker summaries so 12-to-23-month summaries remain comparable with the portfolio they describe.
...
- Decision: `TICKER_MIN_MONTHS` remains 24 for default summaries, but a summary printed under an explicit interactive window uses `min(TICKER_MIN_MONTHS, requested_lookback_months)`.
  Rationale: a summary beside a 12-to-23-month optimized portfolio must measure the ticker under the same eligibility rule. Keeping 24 unconditionally would print `n/a` for every short-window summary; globally lowering it to 12 would weaken ordinary 60-month summaries.
...
Write `tests/test_ticker_stats.py` against real DuckDB files under `tmp_path`, built with `CREATE TABLE` and `INSERT` - the pattern `tests/test_holdings.py` and `tests/test_dataset_upsert.py` already use. Cover: a ticker with full history producing all three figures and the real window dates; a ticker with 12 months declining under the default 24-month minimum but succeeding when an explicit 12-month window supplies `min_months=12`; an 11-month series declining under that same request; a ticker absent from the table declining with a reason; a flat return series declining rather than reporting an enormous Sharpe ratio; a window shorter than `lookback_months` reporting its real start and end; a ticker with dividend coverage reporting a yield; and a ticker without coverage reporting a reason rather than zero. Add one test asserting that `ticker_stats` and `benchmark_stats_for_window` given the identical series, rate, window, and effective minimum produce identical return, volatility and Sharpe figures.
...
`TICKER_MIN_MONTHS` remains 24. `prepare_ticker_summary` passes `min(TICKER_MIN_MONTHS, lookback_months)` only when carrying an explicit interactive window, keeping default and initial summaries unchanged.
...
Revision note (2026-09-22): Added the dynamic minimum-history rule required for ticker summaries shown under the expanded 12-to-60-month interactive window. The default 60/24 summary remains unchanged.
