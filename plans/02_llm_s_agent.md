...
- Decision: add an opt-in `missing_ok: bool = False` parameter to `src/agents/condition_eval.py`'s `evaluate_condition` and `src/agents/llm_s_apply.py`'s `apply_rule`, rather than leaving both untouched and building any missing-factor tolerance as a separate code path elsewhere.
  Rationale: `plans/07_external_candidate_screening.md`'s `screen_external_candidate` needed a way for one candidate factor missing after standardization (e.g. an ETF with no reliable `bm` proxy) to not block an entire rule's `and`/`or` expression from being decided by its other, available clauses — a need this plan's own in-universe screening (`screen()` in `src/agents/llm_s_signals.py`) does not currently have, since every S&P 500 member's `factors` row this plan consumes is expected to have all three factors populated by construction. Rather than fork a second evaluator for plan 07's use case (which would violate this module's own stated design principle of "exactly one code path from an LLM-generated condition string to a boolean result"), both functions gained a default-`False` parameter that leaves every existing call site (this plan's own `screen()`, its tests, `TestComplexConditionTool`) byte-for-byte unaffected — the new three-valued-logic code path is only reachable when a caller explicitly passes `missing_ok=True`, which only `plans/07_external_candidate_screening.md` currently does.
  Date/Author: 2026-08-14, decided in response to the repository owner's request (made while working in `plans/07_external_candidate_screening.md`) that a missing factor degrade gracefully rather than disqualify a candidate entirely; implemented the same day. See that plan's Decision Log for the full design (three-valued/Kleene logic, chosen for its precise, well-established semantics) and verification.
