...
- Decision: within the 60-month/24-month-minimum window above, use whatever number of months a ticker actually has (between 24 and 60) rather than requiring the full 60; only drop a ticker outright if it has fewer than 24.
  ...
  Note (2026-09-06, from `plans/13_user_portfolio.md` Milestone 7): both decisions above still stand as the project's defaults, and the optimizer and candidate pools are untouched. `uv run portfolio-holdings whatif` can now explore a shorter window - 24 to 60 months - for the USER'S OWN HOLDINGS only, because for a holding like `PFF` the choice of window can flip the sign of its Sharpe ratio and a person needs to see more than one. Two consequences for this entry. The 60 remains the default everywhere, so nothing here is superseded. But the tolerance this second decision bought - "use whatever number of months a ticker actually has, between 24 and 60" - has no room to operate at a 24-month window, where the bar equals the window and a holding needs every month in it. That is documented in plan 13's Decision Log rather than engineered around, since the 24 is shared by this module, the benchmark and the holdings report.
