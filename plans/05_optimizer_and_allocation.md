...
- Decision: after calling `ef.efficient_return(target_return=target_monthly_return)` in `compute_weights`, add an explicit post-call sanity check (weights sum to ~1, no NaNs, and `ef.portfolio_performance()`'s realized return is close to `target_monthly_return`) rather than relying solely on letting PyPortfolioOpt's own exception propagate.
  ...
  Date/Author: 2026-08-06, decided by repository owner after reviewing PyPortfolioOpt's own documentation for this plan's next pending implementation step.
