...
half of the summary printed by `src/flow/cli.py`'s `print_ticker_summary`;
the "what will it do to my portfolio" half is computed from this project's
own returns by `src/optimizer/ticker_stats.py`.

`fetch_ticker_profile` is the only function here that performs network I/O,
mirroring `src/dataset/ticker_currency.py`'s `fetch_ticker_currencies`
...
itself (`src/optimizer/dividends.py`), which is additionally the same yield
...
- `fundProfile.feesExpensesInvestment.totalNetAssets` is NOT the fund's net
  ...
  - the pairing `src/agents/external_screen.py`'s `_auto_fetch_etf_aum`
...
Deliberately NOT in `src/config/settings.py`: like
`src/dataset/membership.py`'s `WIKIPEDIA_URL`, this is tied to an external
