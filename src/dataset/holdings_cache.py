...
`src/optimizer/portfolio.py`'s `_load_window_dates` derives a portfolio's
...
`src/optimizer/holdings.py` reports the date of the price it actually used
...
def latest_expected_rebalance_date(as_of: date) -> date:
    ...
    Reuses `src/dataset/membership.py`'s `compute_rebalance_dates` rather
    than hand-rolling a month-start calculation, so the cache's idea of
    "the latest month" is the same one `src/dataset/returns.py` used when it
...
def refresh_holdings_cache(
    ...
    if fresh:
        from src.dataset.ticker_currency import load_ticker_currencies
