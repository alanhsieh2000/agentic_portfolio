...
constraint in `src/agentic_portfolio/optimizer/portfolio.py`.
...
`src/agentic_portfolio/flow/user_portfolio.py`'s `stale_share_counts`).
...
Three layers, the same discipline every other module in `src/agentic_portfolio/dataset/`
...
This module deliberately does NOT reuse `src/agentic_portfolio/dataset/prices.py`'s
...
from agentic_portfolio.dataset.prices import _build_symbol_map, load_latest_close
...
column-less. The same discipline `src/agentic_portfolio/optimizer/portfolio.py`'s
...
Deliberately the same two columns `src/agentic_portfolio/dataset/prices.py` writes to
...
class DividendFieldMissingError(RuntimeError):
    ...
    The counterpart to `src/agentic_portfolio/dataset/prices.py`'s `PriceFieldMissingError`,
...
def _fetch_batch(symbols: list[str], start: str, end: str) -> pd.DataFrame:
    ...
    `auto_adjust=False` matches `src/agentic_portfolio/dataset/prices.py`'s own call so the
...
def probe_served_window(symbol: str) -> tuple[date, date] | None:
    ...
    vaguer warning - the same discipline `src/agentic_portfolio/flow/interactive.py`'s
...
def fetch_dividend_history(
    ...
    results column-wise - mirroring `src/agentic_portfolio/dataset/prices.py`'s
...
def apply_dividend_multipliers(
    ...
    `src/agentic_portfolio/dataset/ticker_currency.py`'s `apply_price_multipliers`, and
...
def upsert_dividends_tables(
    ...
    deleted first, exactly as in `src/agentic_portfolio/dataset/prices.py`'s
...
def load_dividends_long(
    ...
    `src/agentic_portfolio/optimizer/portfolio.py`'s `load_returns_long` where it is opt-in.
...
def split_series_by_ticker(long_df: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    Exists so that `src/agentic_portfolio/dataset/fundamentals.py`'s
...
def announced_amount(stored_amount: float, ex_date: date, splits: pd.Series | None) -> float:
    ...
    Reuses `cumulative_split_ratio_after` (src/agentic_portfolio/dataset/fundamentals.py),
...
def unresolved_frame(
    ...
    `src/agentic_portfolio/dataset/prices.py` cannot make that distinction and says so in its
...
def tickers_with_dividend_data(tickers: list[str], db_path: str = settings.db_path) -> set[str]:
    ...
    `src/agentic_portfolio/dataset/ticker_currency.py`'s `load_ticker_currencies` follows, so
...
def build_dividends_for_tickers(
    ...
    `src/agentic_portfolio/dataset/ticker_ingestion.py` calls so a user-provided ticker gets
...
def build_dividends(
    ...
    its own throwaway snapshot (`src/agentic_portfolio/flow/live.py`'s `build_live_snapshot`,
    which calls this function). `--selection user_provided` fetches per
    typed ticker through `src/agentic_portfolio/dataset/ticker_ingestion.py`. And the
...
def _stored_price_multipliers(tickers: list[str], db_path: str) -> dict[str, float]:
    ...
    `src/agentic_portfolio/dataset/ticker_currency.py`'s `DEFAULT_CURRENCY`) and correctly
...
def main() -> None:
    ...
    `src/agentic_portfolio/dataset/backfill_snapshot.py`'s.
    """
