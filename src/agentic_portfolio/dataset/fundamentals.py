...
CRITICAL: yfinance.download's close/adj_close (used by src/agentic_portfolio/dataset/prices.py)
...
2020-2021, `build_factors` tries src/agentic_portfolio/dataset/sec_edgar.py's SEC EDGAR-based
...
def most_recent_value_on_or_before(shares: pd.Series | None, as_of) -> float | None:
    ...
    Series — reused as-is by src/agentic_portfolio/dataset/momentum.py for price lookups.
    """
...
def attach_nearest_price(membership: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
    ...
    src/agentic_portfolio/dataset/momentum.py calling it once for "1 month before" and once
...
def get_factor_reference_stats(
    ...
    src/agentic_portfolio/agents/external_screen.py and plans/07_external_candidate_screening.md.
