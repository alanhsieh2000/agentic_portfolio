...
Computed directly from the `prices` table (src/agentic_portfolio/dataset/prices.py) — unlike
...
src/agentic_portfolio/dataset/fundamentals.py already wrote (with `mve`/`bm` populated and
...
class FactorsTableMissingError(RuntimeError):
    ...
    extends the table src/agentic_portfolio/dataset/fundamentals.py's build_factors already
...
def load_factors(db_path: str = settings.db_path) -> pd.DataFrame:
    ...
    try:
        ...
        raise FactorsTableMissingError(
            ...
            "`uv run python -m agentic_portfolio.dataset.fundamentals` first."
        ) from e
    finally:
        con.close()
...
def main() -> None:
    ...
    `python -m agentic_portfolio.dataset.momentum`.
    """
