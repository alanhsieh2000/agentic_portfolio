...
downstream carries a unit. `src/agentic_portfolio/optimizer/portfolio.py`'s weights are immune
...
I/O, mirroring `src/agentic_portfolio/dataset/prices.py`'s `_fetch_batch` discipline, so it is
...
three LLM selections, `src/agentic_portfolio/dataset/backfill_snapshot.py`, the backtest
...
class MixedCurrencyPoolError(UnsatisfiableRequestError):
    ...
    Deliberately a `ValueError` subclass: `src/agentic_portfolio/flow/cli.py`'s `_run_edit_loop`
...
def fetch_ticker_currencies(
    ...
    `src/agentic_portfolio/dataset/prices.py` already makes does not carry a currency, and
...
def upsert_ticker_currency_table(currency_df: pd.DataFrame, tickers: list[str], db_path: str) -> None:
    ...
    `src/agentic_portfolio/dataset/prices.py`'s `upsert_prices_tables` uses, and driven by the
...
def load_ticker_currencies(tickers: list[str], db_path: str) -> dict[str, str]:
    ...
    Opened read-only, following `src/agentic_portfolio/dataset/fundamentals.py`'s
...
def apply_price_multipliers(long_prices: pd.DataFrame, multipliers: dict[str, float]) -> pd.DataFrame:
    ...
    since `src/agentic_portfolio/optimizer/portfolio.py`'s `load_latest_prices` reads it.
    ...
    weight, volatility or Sharpe ratio: `src/agentic_portfolio/dataset/returns.py` computes a
...
def partition_by_currency(
    ...
    `src/agentic_portfolio/flow/interactive.py`'s `validate_and_edit_candidates`).
