...
snapshot, but `src/agentic_portfolio/dataset/membership.py`'s `compute_rebalance_dates` only
ever produces rows from `settings.rebalance_start` (2020-01-01) onward, so
`src/agentic_portfolio/agents/llm_s.py`'s `resolve_as_of_date(2020)` fell back to 2020-01-01
...
from agentic_portfolio.dataset.membership import apply_changes_asof, fetch_and_normalize_membership
from agentic_portfolio.dataset.momentum import compute_mom12m_column
...
def main() -> None:
    ...
    `python -m agentic_portfolio.dataset.backfill_snapshot`. Takes an optional ISO date as
