...
`src/agentic_portfolio/flow/report_summary.py` reads a month of the saved report archive and
...
The split is the same one `src/agentic_portfolio/agents/llm_f.py` makes and for the same reason.
...
`src/agentic_portfolio/agents/summary_schema.py` reach the model as its output schema; the
backstory in `src/agentic_portfolio/agents/summary_crew/config/agents.yaml` states the rule in
...
class NarrativeUnavailable(RuntimeError):
    ...
    by accident; `src/agentic_portfolio/flow/summary_cli.py` catches it and prints the computed
...
def generate_narrative(
    ...
    `facts` is `src/agentic_portfolio/flow/report_summary.py`'s `digest_for_llm` output: the
