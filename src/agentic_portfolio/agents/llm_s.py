...
from agentic_portfolio.agents.llm_s_crew.crew import LLMSCrew
from agentic_portfolio.agents.llm_s_crew.tools import load_snapshot
...
def resolve_as_of_date(year: int, db_path: str = settings.db_path) -> date:
    ...
    2019-12-31 row in `factors`, which `src/agentic_portfolio/dataset/backfill_snapshot.py`
    ...
    Decision Log) - run `uv run python -m agentic_portfolio.dataset.backfill_snapshot`
