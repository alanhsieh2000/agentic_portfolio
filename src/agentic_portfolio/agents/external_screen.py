...
(`src/agentic_portfolio/dataset/fundamentals.py`'s `add_cross_sectional_z`). Applying that
...
(`src/agentic_portfolio/dataset/fundamentals.py`) recovers that S&P 500 mean/std on demand
...
from agentic_portfolio.dataset.momentum import compute_mom12m
from agentic_portfolio.dataset.prices import fetch_price_history, reshape_prices_long, to_yfinance_symbol
...
def _load_price_series(ticker: str, as_of_ts: pd.Timestamp) -> pd.Series:
    ...
    `src/agentic_portfolio/dataset/prices.py`'s S&P-500-membership pipeline calls, just for
...
def compute_raw_factors_for_ticker(ticker: str, as_of_date) -> dict[str, float | None]:
    ...
    `src/agentic_portfolio/dataset/fundamentals.py`'s `build_factors` and
    `src/agentic_portfolio/dataset/momentum.py`'s `build_momentum_factors` already call per
...
def screen_external_candidate(
    ...
    (`src/agentic_portfolio/agents/llm_s_apply.py`) the in-universe `screen()` function uses.
