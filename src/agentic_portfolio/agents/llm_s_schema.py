...
`ScreeningRule` is what one `generate_rule` call (src/agentic_portfolio/agents/llm_s.py)
...
src/agentic_portfolio/agents/llm_s/config/) is kept verbatim from the paper and never
mentions a `_z` suffix. Syntax/name validation of the condition strings
happens in src/agentic_portfolio/agents/condition_eval.py, not here — this model only
...
class ScreeningRule(BaseModel):
    ...
    (which is December of `year - 1`, per src/agentic_portfolio/agents/llm_s.py).
    """
