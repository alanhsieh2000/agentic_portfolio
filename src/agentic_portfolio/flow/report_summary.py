...
`src/agentic_portfolio/flow/report_archive.py` writes one file per printed report under
...
around those figures (`src/agentic_portfolio/agents/report_summary.py`) is handed them
already computed and is forbidden from doing arithmetic of its own. That
is the same split `src/agentic_portfolio/agents/llm_f.py` makes, where the model estimates
...
from agentic_portfolio.flow.report_archive import load_report, report_digest
...
class SourceReport:
    ...
    also the token `report_filename` in `src/agentic_portfolio/flow/report_archive.py` builds the
