...
(`load_latest_prices`, which delegates to `src/agentic_portfolio/dataset/prices.py`'s
...
def load_returns_long(
    ...
    src/agentic_portfolio/dataset/returns.py's build_month_ticker_grid), so every calendar
    ...
    what `src/agentic_portfolio/optimizer/benchmark.py` reads once per session so that every
    ...
    it - the same discipline `src/agentic_portfolio/dataset/ticker_currency.py`'s
...
def load_returns_matrix_unfiltered(
    ...
    `src/agentic_portfolio/optimizer/holdings.py`, which subtracts this matrix's columns from
...
def load_returns_matrix(
    ...
    (src/agentic_portfolio/dataset/returns.py) and filtered by the minimum-history drop rule
...
def _estimate_mu_and_cov(returns_matrix: pd.DataFrame) -> tuple[pd.Series, pd.DataFrame]:
    ...
    chose (a user's actual holdings, see `src/agentic_portfolio/optimizer/holdings.py`). Those
...
def _fit_efficient_frontier(
    ...
    try:
        ...
        # `src/agentic_portfolio/flow/cli.py`'s `except ValueError` and take live mode's only
        ...
        # person is told what to change - and so `src/agentic_portfolio/flow/cli.py` can catch
...
class UnreachableTargetReturnError(UnsatisfiableRequestError):
    ...
    `src/agentic_portfolio/optimizer/benchmark.py`'s objective resolution clamps a
...
def compute_weights(
    ...
    `src/agentic_portfolio/flow/backtest.py` scores a whole 52-month run by computing its own
...
class PortfolioStats(NamedTuple):
    ...
    display decision, made by `src/agentic_portfolio/flow/cli.py`. `risk_free_rate` and
    ...
    `src/agentic_portfolio/dataset/dividends.py`'s `load_dividend_figures`. It is echoed
...
def compute_weights_and_stats(
    ...
    *,
    ...
    to buy - see `src/agentic_portfolio/flow/cli.py`'s `print_weights_and_allocation`.
...
def stats_for_weights(
    ...
    turned into value shares by `src/agentic_portfolio/optimizer/holdings.py` - and only the
    ...
    `src/agentic_portfolio/optimizer/benchmark.py`'s module docstring for why a single-column
...
def latest_price_date(tickers: list[str], as_of: date, db_path: str) -> date | None:
    ...
    persistent cache (see `src/agentic_portfolio/dataset/holdings_cache.py`) the date stops
...
def load_latest_prices(tickers: list[str], as_of: date, db_path: str = settings.db_path) -> pd.Series:
    ...
    A thin delegation to `src/agentic_portfolio/dataset/prices.py`'s `load_latest_close`,
    ...
    signature means `allocate_shares`, `src/agentic_portfolio/optimizer/holdings.py`'s
    `weights_from_positions` and `src/agentic_portfolio/flow/interactive.py` all keep pricing
