...
figure here is produced by the very estimators `src/agentic_portfolio/optimizer/portfolio.py`
...
rows may be written - lives in `src/agentic_portfolio/flow/interactive.py`'s
`prepare_benchmark`, keeping this layer free of both `src/agentic_portfolio/flow` and yfinance.
"""
...
from agentic_portfolio.optimizer.portfolio import load_returns_long
...
is ASKED for instead (see `src/agentic_portfolio/flow/cli.py`'s candidate-pool confirm loop),
...
The same bar `src/agentic_portfolio/optimizer/portfolio.py`'s `apply_min_history_rule` sets for
...
def load_ticker_monthly_returns(ticker: str, as_of: date, db_path: str) -> pd.Series:
    ...
    `src/agentic_portfolio/optimizer/ticker_stats.py` needs one candidate's history in exactly
...
def annualized_return_and_volatility(monthly_returns: pd.Series) -> tuple[float, float]:
    ...
    using the identical estimators `src/agentic_portfolio/optimizer/portfolio.py` applies to a
    ...
    the monthly cadence `src/agentic_portfolio/dataset/returns.py` produces.
...
def objective_from_benchmark(
    ...
    caller (see `src/agentic_portfolio/flow/interactive.py`), which reports it back through
    `clamped_from`.
    """
