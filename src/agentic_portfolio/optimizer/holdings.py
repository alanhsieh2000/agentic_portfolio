...
`memory/portfolio.json` (see `src/agentic_portfolio/flow/user_portfolio.py`) into value
...
The distinction from `src/agentic_portfolio/optimizer/portfolio.py` is which direction the
...
with the optimizer and with `src/agentic_portfolio/optimizer/benchmark.py`, so a held
...
`src/agentic_portfolio/optimizer/benchmark.py`'s `BENCHMARK_MIN_MONTHS` already apply, named
...
data rather than a policy: `src/agentic_portfolio/dataset/ticker_ingestion.py` fetches
...
def validate_lookback_months(months: object, source: str) -> int:
    ...
    Modelled on `src/agentic_portfolio/flow/rate_memory.py`'s `validate_risk_free_rate`: takes
...
class HoldingsStats(NamedTuple):
    ...
    Follows `src/agentic_portfolio/optimizer/benchmark.py`'s `BenchmarkStats` discipline:
    ...
    `src/agentic_portfolio/dataset/holdings_cache.py`), so a total can legitimately be weeks
    ...
    match `src/agentic_portfolio/optimizer/portfolio.py`'s `PortfolioStats` fields so the
...
def unavailable_holdings(
    ...
    Public because `src/agentic_portfolio/flow/interactive.py`'s `prepare_holdings` needs it
...
def holdings_stats(
    ...
    `src/agentic_portfolio/flow/interactive.py`'s `prepare_holdings`, kept separate for the
    ...
    `src/agentic_portfolio/dataset/dividends.py`'s `load_dividend_figures`, resolved by the
