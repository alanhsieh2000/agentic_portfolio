...
`src/optimizer/benchmark.py`'s `DEFAULT_BENCHMARKS`. `SPY` is defensible as
...
imports nothing from `src/config`, `src/optimizer`, or any other `src/flow`
...
def resolve_risk_free_rate(
    ...
    `src/optimizer/benchmark.py`'s `resolve_benchmark_ticker` uses, and what
    keeps this module free of a `src.config.settings` import.
