...
Why this exists. Both `uv run portfolio` (`src/flow/cli.py`) and
`uv run portfolio-holdings whatif` (`src/flow/holdings_cli.py`) answer "how
...
produced by roughly 109 `print()` calls in `src/flow/cli.py`; there is no
...
byte-identical output (`src/flow/holdings_cli.py` says a default run's report
...
"""Fallback for `src/config/settings.py`'s `output_dir`, which is the
...
def _write_atomically(path: Path, text: str) -> None:
    ...
    Copied from `src/flow/rate_memory.py`'s `_write_rates`, the project's one
