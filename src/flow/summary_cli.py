...
Every figure in the output is computed by `src/flow/report_summary.py` from the
saved front matter. The prose between the tables is written by
`src/agents/report_summary.py` on the cheap model named by `LLM_QUICK`, and is
...
from src.agents.report_summary import NarrativeUnavailable, generate_narrative
from src.agents.summary_schema import MonthNarrative
...
def narrative_prose(narrative: MonthNarrative | None) -> dict[str, str]:
    ...
    side of the seam on purpose: `src/flow/report_summary.py` renders text it
