...
def apply_rule(rule: ScreeningRule, factor_row: dict, missing_ok: bool = False) -> str:
    ...
    `missing_ok` is forwarded to `evaluate_condition` unchanged (default
    False, preserving this function's original behavior exactly); see its
    docstring for what setting it True does with a factor absent from
    `factor_row`.
    """
    if evaluate_condition(rule.buy_condition, factor_row, missing_ok=missing_ok):
        return "buy"
    if evaluate_condition(rule.sell_condition, factor_row, missing_ok=missing_ok):
        return "sell"
    return "hold"
