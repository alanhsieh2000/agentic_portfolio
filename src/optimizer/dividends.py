...
Everything here is pure - no DuckDB, no yfinance, nothing from `src/flow` -
so `src/optimizer/portfolio.py` can type its new parameters against these
...
`src/flow/rate_memory.py`'s `MAX_ABS_RISK_FREE_RATE = 1.0` reasoning applied
...
def validate_dividend_yield(value: object, source: str) -> float:
    ...
    Modelled on `src/flow/rate_memory.py`'s `validate_risk_free_rate`, and
...
def covered_dividend_yield(
    ...
    `src/optimizer/holdings.py` already applies to a holding it cannot
