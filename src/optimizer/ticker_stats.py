...
This is the second half of the summary `src/flow/cli.py`'s
`print_ticker_summary` prints when somebody adds a ticker to a
`user_provided` candidate pool. The first half, in
`src/dataset/ticker_profile.py`, is what Yahoo Finance publishes about the
...
The reason is comparability, the same argument `src/optimizer/benchmark.py`
...
imports nothing from `src/flow` and nothing from `yfinance`, following
`src/optimizer/holdings.py`, which is what makes it testable against a
fixture database with no mocking at all. Deciding whether a ticker needs
fetching first, and into which database, is `src/flow/interactive.py`'s
...
`src/optimizer/benchmark.py`'s docstring, one holding moved from 0.1614
...
The same trailing window `src/optimizer/portfolio.py`'s
...
Deliberately the same bar `src/optimizer/portfolio.py`'s
`apply_min_history_rule` sets for a pool ticker and
`src/optimizer/benchmark.py`'s `BENCHMARK_MIN_MONTHS` sets for a benchmark.
...
def _dividend_figures(
    ...
    Reuses `src/dataset/dividends.py`'s `load_dividend_figures`, which is
...
def ticker_stats(
    ...
    imported from `src/optimizer/benchmark.py` rather than reimplemented.
    ...
    in `src/optimizer/benchmark.py`: a flat series is a data problem rather
