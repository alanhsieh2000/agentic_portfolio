...
Deliberately at the top of `src/` with NO imports of its own. The types
here are raised in the dataset layer (`src/dataset/ticker_currency.py`), in
the optimizer (`src/optimizer/portfolio.py`,
`src/optimizer/dividends.py`) and caught in the flow layer
(`src/flow/cli.py`), so anywhere else would create an import cycle for at
least one of them.
"""
...
class UnsatisfiableRequestError(ValueError):
    ...
    bugs, and `src/flow/cli.py`'s `main` is exactly where that distinction
    has to hold.

    Subclasses `ValueError` so that every existing handler keeps working -
    notably `src/flow/cli.py`'s interactive edit loop, which reverts a
